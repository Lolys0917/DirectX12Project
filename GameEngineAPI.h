...
namespace EngineGame
{
    namespace Detail
    {
        ...
        {
            if (ActiveHost == nullptr || ActiveSceneID == 0 || objectID == 0 ||
                ActiveHost->GetObjectCount == nullptr ||
                ActiveHost->GetObjectInfo == nullptr)
            {
                return false;
            }

            const std::uint32_t Count = ActiveHost->GetObjectCount(
                ActiveHost->Context,
                ActiveSceneID
            ); //現在SceneのObject数
    ...
    public:
        ...
        std::uint32_t CreateCapsuleModel(
            const std::string& name,
            std::uint32_t parentObjectID = 0
        ) const
        {
            ...
            if (Detail::HasHostMember(Host, CapsuleMemberEnd) &&
                ...
                !name.empty())
            {
                return Host->CreateCapsuleModel(
                    Host->Context,
                    SceneID,
                    name.c_str(),
                    parentObjectID
        ...
        std::uint32_t CreateBox(const std::string& name) const
        {
            return Create(EngineExternalObjectType::Box, name);
        }

        std::uint32_t CreateSphere(const std::string& name) const
        {
            return Create(EngineExternalObjectType::Sphere, name);
        }

        std::uint32_t CreatePlane(const std::string& name) const
        {
            return Create(EngineExternalObjectType::Plane, name);
        }

        std::uint32_t CreateCylinder(const std::string& name) const
        {
    ...
    public:
        ...
        std::uint32_t CreateCapsuleModel(const std::string& name) const
        {
            return AddObjectAPI(
                Detail::ActiveHost,
    ...
    public:
        ...
        {
            return Detail::FindObjectID(name);
        }
        ...
        {
            const std::uint32_t ObjectID = Find(name); //Transform対象ID
        ...
        {
            const std::uint32_t ObjectID = Find(name); //位置変更対象ID
        ...
        {
            const std::uint32_t ObjectID = Find(name); //移動対象ID
        ...
        {
            const std::uint32_t ObjectID = Find(name); //色設定対象ID
        ...
        {
            const std::uint32_t ObjectID = Find(name); //色乗算対象ID
        ...
        {
            const std::uint32_t ObjectID = Find(name); //削除対象ID
            return Detail::ActiveHost != nullptr && ObjectID != 0 &&
        ...
        {
            const std::uint32_t ObjectID = Find(name); //接続対象ID
            return Detail::ActiveHost != nullptr && ObjectID != 0 &&
